...
def run_test(test: str = "all", N: int = 300, plot: bool = False):
    """
        ...
        Args:
        test: The name of the test to be run. Default "all", performs all tests
        plot: If True, will create and show plots

        """

    name = "Lamb3D_inline"
    if test == name or test == "all":
        print("Testing: " + name + " ....... ", end='')
        lamb3D_test(testtype="inline", plots=plot, N=N)

    name = "Lamb3D_crossline"
    if test == name or test == "all":
        print("Testing: " + name + " ....... ", end='')
        lamb3D_test(testtype="crossline", plots=plot, N=N)

    name = "Garvin_2D"
    if test == name or test == "all":
        print("Testing: " + name + " ....... ", end='')
        garvin2D_test(plots=plot, N=N)
    ...
    if test == name or test == "all":
        print("Testing: " + name + " ....... ", end='')
        homogeneous_test(testname=name, testtype="inline", plots=plot,
                         taup=0.1, taus=0.1, N=N)

    name = "viscoelastic_3D_crossline"
    if test == name or test == "all":
        print("Testing: " + name + " ....... ", end='')
        homogeneous_test(testname=name, testtype="crossline", plots=plot,
                         taup=0.1, taus=0.1, N=N)
    ...
    if test == name or test == "all":
        ...
        homogeneous_test(testname=name, ND=2,
                         testtype="crossline", plots=plot, N=N)

    name = "elastic_2D_inline"
    if test == name or test == "all":
        print("Testing: " + name + " ....... ", end='')
        homogeneous_test(testname=name, ND=2,
                         testtype="inline", plots=plot, N=N)

    name = "viscoelastic_2D_crossline"
    if test == name or test == "all":
        print("Testing: " + name + " ....... ", end='')
        homogeneous_test(testname=name, ND=2,
                         testtype="crossline", plots=plot,
                         taup=0.2, taus=0.2, N=N)

    name = "viscoelastic_2D_inline"
    if test == name or test == "all":
        print("Testing: " + name + " ....... ", end='')
        homogeneous_test(testname=name, ND=2,
                         testtype="inline", plots=plot,
                         taup=0.2, taus=0.2, N=N)
...
if __name__ == "__main__":
    ...
    plot = args.plot
    N = args.N
    run_test(test=name, N=N, plot=plot)
